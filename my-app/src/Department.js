...
export const Department=()=>{
    const [departmentOnDisplay,setDepartmentOnDisplay]=useState([]);
    ...
    //FILTERING : Just Set onChange={(e)=>setDeptNameFilter(e.target.value)} in input box for Triggering effect
    useEffect(()=>{
        var filteredData= departmentOriginal.filter(
            function (el){
        ...
        setDepartmentOnDisplay(filteredData);
    
    },[deptIdFilter,deptNameFilter])

    //SORTING: Just use onClick={()=>sortResult('DepartmentName',false)} for the button
    ...
    const readAllRequest=()=>{
        ...
        .then((response)=>{
            setDepartmentOnDisplay(response.data);
            setDepartmentOriginal(response.data);
         });
    }
    const updateRequest=()=>{
        let data={
    ...
    return(<>
        ...
        <table>
                ...
                <tr>
                    <th>
                        <div className="d-flex flex-row">                       
                            <input className="form-control m-2" onChange={(e)=>setDeptIdFilter(e.target.value)} placeholder="IdFilter"/>
            ...
            <tbody>{/*****************************Record mapping****************************/}
                {departmentOnDisplay.map(dept=>{return (                
                <tr>
